...
public class SimpleTest {
	...
	TestInputStream proxyInputStream;
	TestInputStream serverInputStream;
	TestInputStream clientInputStream;

	@Before
	public void before() throws Exception {
		proxyInputStream = new TestInputStream();
		serverInputStream = new TestInputStream();
		clientInputStream = new TestInputStream();

		proxy = componentFactory.startProxy(new Config("proxy"), new Shell("proxy", new TestOutputStream(System.out), proxyInputStream));
		Thread.sleep(Util.WAIT_FOR_COMPONENT_STARTUP);

		server = componentFactory.startFileServer(new Config("fs1"), new Shell("fs1", new TestOutputStream(System.out), serverInputStream));
		Thread.sleep(Util.WAIT_FOR_COMPONENT_STARTUP);

		client = componentFactory.startClient(new Config("client"), new Shell("client", new TestOutputStream(System.out), clientInputStream));
		Thread.sleep(Util.WAIT_FOR_COMPONENT_STARTUP);
	}
